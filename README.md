...
- **侧边栏脚注「重启」按钮**：宽栏显示图标 + 文字，窄栏仅图标；点击后弹确认框（支持 Esc 关闭），确认后重启服务。
- **设置页「快速重启」卡片**：显示服务 PID、重启日志路径、重启按钮与状态提示。
...
- **原生重启页**：确认重启后脱离 React 原生接管页面——旋转动画 + 阶段文案（停止旧进程 → 启动新进程 → 等待就绪）+ 进度条，明暗主题自动适配；多标签页通过 BroadcastChannel 同步进入重启屏；恢复后展示「重启完成」提示。
- **更稳的恢复判定**：必须观察到服务下线（旧进程确认退出）后再持续在线 8 秒，才判定真正恢复并刷新，避免把退出宽限期内的旧服务误判为已恢复。
- **自适应进度与耗时统计**：记录上次重启的实际总耗时，下次重启的进度条节奏与「比预期慢」提示按历史耗时自适应；完成后 toast 显示本次耗时。
- **刷新安全**：重启期间误刷新或重开页面（恢复后加载成功即证明服务已回来）会自动清理重启标记并展示「重启完成」提示，不落入断连困惑；标记超过 5 分钟自动过期。
- **失败排查引导**：超时或请求未送达时，重启页直接展示日志路径并支持一键复制，不用翻文档找日志位置。
- **重启防卡死**：host 端「重启已在进行中」标志带 15 秒兜底窗口，上次重启异常未退出时不会永久拒绝后续重启请求。
