...
/** 重启日志路径（status 返回给客户端，失败时引导排查用）。 */
const restartLogPath = () => join(dshHome(), 'logs', 'dsh-quick-restart.log')

/**
 * 「重启已在进行中」的有效窗口（毫秒）。正常流程下 restart 请求约
 * EXIT_GRACE_MS 后进程即退出，窗口内重复请求会被拒绝；若进程因异常
 * 未退出，窗口过期后允许再次发起重启——否则 restarting 标志会永久
 * 卡死，恰好废掉本插件的核心功能。
 */
const RESTARTING_WINDOW_MS = 15_000

export function apply(ctx) {
  const log = ctx.logger ?? console
  let restartingAt = 0 // 最近一次重启的发起时刻（0 = 空闲）

  const isRestarting = () => restartingAt > 0 && Date.now() - restartingAt < RESTARTING_WINDOW_MS
  ...
  const scheduleRestart = () => {
    ...
    const restartLog = restartLogPath()
  ...
  const handler = async (endpoint, payload = {}) => {
    if (endpoint === ENDPOINTS.status) {
      return { ok: true, value: { pid: process.pid, restarting: isRestarting(), logPath: restartLogPath() } }
    }
    if (endpoint === ENDPOINTS.restart) {
      ...
      // 窗口内的重复请求拒绝；窗口过期（上次重启异常未退出）则放行重试
      if (isRestarting()) return fail('重启已在进行中 | restart already in progress')
      try {
        scheduleRestart()
