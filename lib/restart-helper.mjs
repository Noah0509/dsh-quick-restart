...
// 自身 stdout/stderr 由宿主重定向进 logPath，任何崩溃/异常都可查。
...
/** 参数校验失败时向 stderr 说明原因并以非零码退出（宿主已把 stderr 重定向进日志）。 */
function bail(message) {
  console.error(`dsh-quick-restart helper: ${message}`)
  process.exit(1)
}

const pid = Number(oldPid)
if (!Number.isInteger(pid) || pid <= 0) bail(`invalid oldPid: ${String(oldPid)}`)
if (typeof cwd !== 'string' || cwd.length === 0) bail('missing cwd')
if (typeof logPath !== 'string' || logPath.length === 0) bail('missing logPath')
if (cmdArgs.length === 0) bail('missing relaunch command args')
...
// 给端口释放留一点余量（进程退出即关闭全部句柄，200ms 足够）
await sleep(200)
